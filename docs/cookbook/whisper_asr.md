...
runtime_overrides:
  asr:
    ...
    encoder_graph_batch_buckets: [1, 2, 4, 8, 12, 16]
```

The graph is captured after SGLang's generation graphs. Each request uses the smallest captured bucket that fits its batch.
Requests larger than every captured bucket, with a different feature shape, or without a successful capture run eagerly. Startup and first-replay logs identify the captured and executed buckets.
